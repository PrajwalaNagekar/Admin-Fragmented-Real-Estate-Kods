import { TrendingUp, TrendingDown, Users, Building2, Eye, IndianRupee, AlertTriangle, Clock } from "lucide-react";
...
const kpiCards = [
  { title: "Total Portfolio Value", value: "₹12.5 Cr", change: "+12.5%", trend: "up", icon: IndianRupee },
  { title: "Active Users", value: "1,240", change: "+8.2%", trend: "up", icon: Users },
  { title: "Listed Properties", value: "45", change: "+3", trend: "up", icon: Building2 },
  { title: "Total Views", value: "85.2K", change: "-2.1%", trend: "down", icon: Eye },
];
...
const COLORS = ["hsl(215,75%,50%)", "hsl(210,40%,96%)"];
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={chartData.portfolioValue}>
                ...
                <YAxis stroke="hsl(215.4,16.3%,46.9%)" fontSize={12} tickFormatter={(v) => `₹${(v / 10000000).toFixed(1)}Cr`} />
                <Tooltip contentStyle={{ backgroundColor: "hsl(0,0%,100%)", border: "1px solid hsl(214.3,31.8%,91.4%)", borderRadius: "8px", color: "hsl(222.2,84%,4.9%)" }} formatter={(v: number) => [`₹${(v / 10000000).toFixed(2)} Cr`, "Value"]} />
          ...
          <CardContent className="space-y-3">
            {alerts.map((alert, i) => (
              <div key={i} className="flex items-start gap-3 py-2 border-b border-border last:border-0">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${alert.type === "warning" ? "bg-warning/10" : alert.type === "error" ? "bg-destructive/10" : "bg-info/10"
                  }`}>
